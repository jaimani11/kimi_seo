...
describe('gobookt sitemap index', () => {
  it('exposes the expected named sections in order', () => {
    expect(sitemapSectionNames()).toEqual(['core', 'destinations', 'hotels', 'occasion-stays', 'editorial']);
  });
  ...
  it('every URL uses the canonical origin host — no wrong-host, no cross-brand', () => {
    ...
    for (const sibling of ['gotript', 'numiworks', 'stayviaowner']) {
  ...
  it('parity: the split preserves every programmatic URL (nothing dropped)', () => {
    ...
    for (const slug of enumerateHotelTypeSlugs()) expect(urlset.has(`${origin}/hotels/${slug}`)).toBe(true);
  ...
  it('renders a valid sitemap index + valid child urlsets', () => {
    ...
    expect(idx).toContain(`${origin}/sitemaps/hotels.xml`);
    expect(idx).not.toContain('<?xml-stylesheet'); // an index is machine-only

    const child = renderSitemapXml(sitemapSectionEntries('occasion-stays') ?? []);
