...
Multi-category Booking.com affiliate travel hub — hotels, flights,
...
- Booking.com Partners affiliate program (multi-vertical)
...
| Name | What it is |
|------|------------|
| `BOOKING_COM_AFFILIATE_ID` | Your Booking.com `aid` (sub-account id). Without it the URLs still work, attribution doesn't track. |
| `BOOKING_COM_AFFILIATE_LABEL` | Optional sub-channel label. Defaults to `gobookt`. |
| `NEXT_PUBLIC_STAYSCOUT_ACTIVE_STAY_PROVIDER` | Already defaults to `booking-com` in code. Override only to fall back to viator/expedia. |
...
2. Builds the right Booking.com URL via
   `lib/affiliate/booking-com-multicategory.ts`
3. Attaches the affiliate id, label, and `_src=gobookt`
4. 302s the browser to Booking.com in a new tab
...
+ restructured for the multi-category Booking.com affiliate program.
