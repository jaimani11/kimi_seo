 ...
 *                                     a specific product)
 *   - dpl_turn=<turnId?>             (gobookt-private — round-trips
...
export function decorateOutboundUrl(url: string, opts: DecorateOutboundOptions): string {
  ...
  setIfAbsent(parsed.searchParams, 'utm_source', 'gobookt');
