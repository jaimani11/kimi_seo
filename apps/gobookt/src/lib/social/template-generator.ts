...
const CTA_OPTIONS = [
  'Plan your trip on gobookt.com',
  'Build the itinerary at gobookt.com',
  'Free AI trip planner → gobookt.com',
  'Get the full guide on gobookt.com',
  'Bookable on Viator via gobookt.com',
];
  ...
  if (guide) {
    ...
    pins.push({
      ...
      description: guide.neighborhoods
        ...
        .join(' ')
        .slice(0, 480),
      visualConcept: `Stylized map of ${city.name} with neighborhoods highlighted in gobookt brand colors, plus a small landmark icon per neighborhood.`,
