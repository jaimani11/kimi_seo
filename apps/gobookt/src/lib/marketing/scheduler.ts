  ...
  await Promise.all(
    platforms.map(async (platform) => {
      ...
      for (let i = 0; i < slugs.length; i++) {
        ...
        // Rewrite the CTA to embed the branded gobookt.com URL +
...
/**
 * Rewrite the CTA on a payload to point at gobookt.com with a
 * UTM-tagged link. The pack generator's templates and the LLM both
 * include gobookt.com mentions in the CTA text, but we
...
function withBrandedCta(
  ...
  const brandedUrl = brandedGobooktUrl(args);
  const original = (payload as { cta?: string }).cta ?? '';
  // Drop any previously-attached URL on the CTA — the templates
  // sometimes embed gobookt.com without UTM. Re-attach the
  // tagged URL fresh.
  const stripped = original.replace(/\s*(https?:\/\/)?(www\.)?gobookt\.com[^\s]*$/i, '').trim();
