/**
 * Branded gobookt.com URLs for social-content CTAs.
 *
 * Every post that goes out on Pinterest, Instagram, or TikTok must
 * route back to gobookt.com — both for brand presence and for
 ...
 * — that's the page gobookt owns, monetizes (via the Viator
 * affiliate links on the page), and can iterate on.
 */

const ORIGIN = 'https://www.gobookt.com';

export interface BrandedUrlInput {
  citySlug: string;
  platform: 'pinterest' | 'instagram' | 'tiktok';
  /** Optional themed path slug (e.g. `paris-with-kids`). When present
   *  the URL points at `/{pathSlug}` instead of the default
   *  `/destinations/{citySlug}` — used for themed pins that have their
   *  own landing page. */
  pathSlug?: string;
}

export function brandedGobooktUrl(input: BrandedUrlInput): string {
...
/**
 * Compact label used inside CTA copy ("Plan it on gobookt.com").
 ...
 */
export const GOBOOKT_DOMAIN_LABEL = 'gobookt.com';
