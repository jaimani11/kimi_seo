...
import { CATEGORY_META, type BookingComCategory } from '@lib/affiliate/booking-com-multicategory';

/**
 * Gobookt multi-category search hero.
 *
 * Visual sister to numiworks's SearchFormHero — same Booking.com /
 ...
 *
 * On top of that base, gobookt's 5-tab category strip — Stays /
 * Flights / Things to do / Cars / Cruises — sits above the field
 * grid so a single hero search covers every Booking.com vertical.
 */
...
/** Cruise embarkation ports — substituted into the destination
 *  autocomplete + Popular chips when the Cruises tab is active. */
const POPULAR_CRUISE_PORTS = [
  'Miami',
  'Barcelona',
  'Seattle',
  'Athens',
...
export function MultiCategoryHero({
  ...
  initialCategory?: BookingComCategory;
} = {}) {
  const today = new Date();
  const defaultIn = isoDate(addDays(today, 14));
  const defaultOut = isoDate(addDays(today, 17));

  const [category, setCategory] = useState<BookingComCategory>(initialCategory);
  ...
  // Cruises now route to Booking.com Hotels at the embarkation port,
  // so we show the full hotel-shaped form (dates + travelers).
  const showDates = true;
  const showTravelers = category === 'hotels' || category === 'flights' || category === 'cruises';
  const destinationOptions =
    category === 'cruises' ? POPULAR_CRUISE_PORTS : POPULAR_DESTINATIONS;
  const popularChipLabel = category === 'cruises' ? 'Popular ports' : 'Popular';
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Tab strip — sits flush against the form like Booking.com's
            * own search */}
          <div
            style={{
              background: PANEL_BG,
          ...
          <div
            style={{
              ...
              overflow: 'hidden',
              display: 'grid',
              gridTemplateColumns: gridTemplateFor(showDates, showTravelers),
            }}
          >
        ...
        >
          Search hands off to <strong style={{ fontWeight: 700, color: '#ffd166' }}>Booking.com</strong>. Affiliate link;
...
function gridTemplateFor(showDates: boolean, showTravelers: boolean): string {
  // Build a sensible grid template per category. Always reserve a
  // column for the Search button at the end.
  const cols: string[] = ['minmax(0, 1.6fr)']; // destination
  if (showDates) {
...
function headlineFor(c: BookingComCategory): string {
  switch (c) {
    case 'flights':
    ...
    case 'cruises':
      return 'Cruise port hotels — book your pre & post-cruise stay';
    case 'attractions':
      return 'Tours, day trips, food walks & attraction tickets';
    default:
      return 'Find hotels, flights & things to do worldwide';
  }
}

function subheadFor(c: BookingComCategory): string {
  const tail = 'Powered by Booking.com. Real-time prices, free cancellation on most bookings.';
  switch (c) {
    case 'flights':
    ...
    case 'cruises':
      return `Miami, Barcelona, Seattle, Athens, Southampton — hotels at every major embarkation port. ${tail}`;
    case 'attractions':
      return `Skip-the-line tickets, guided tours, half-day adventures. ${tail}`;
    default:
      return `Search across hotels, flights, cars, cruises and bookable experiences. ${tail}`;
  }
}

function destinationLabelFor(c: BookingComCategory): string {
  switch (c) {
    case 'flights':
      return 'Flying to';
    case 'cars':
...
function destinationPlaceholderFor(c: BookingComCategory): string {
  switch (c) {
    case 'flights':
      return 'e.g. Tokyo';
    case 'cars':
    ...
    case 'cruises':
      return 'e.g. Miami, Barcelona, Seattle';
    case 'attractions':
      return 'e.g. Paris';
    default:
      return 'Where to?';
...
function dateLabelFor(c: BookingComCategory): { checkIn: string; checkOut: string } {
  switch (c) {
    case 'flights':
    ...
    case 'cruises':
      return { checkIn: 'Pre-cruise check-in', checkOut: 'Check-out' };
    default:
