...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (parsed.kind === 'best-time') {
    const title = `Best Time to Visit ${parsed.city.name}: Month by Month · gotript`;
  ...
  if (parsed.kind === 'weather-month') {
    ...
    const title = `${parsed.city.name} Weather in ${monthName(parsed.monthIndex)} · gotript`;
    const description = m
  ...
  if (parsed.kind === 'where-to-stay') {
    const title = `Where to Stay in ${parsed.city.name}: Best Areas & Neighborhoods · gotript`;
  ...
  if (parsed.kind === 'where-to-go-month') {
    const title = `Where to Go in ${monthName(parsed.monthIndex)}: Destinations Ranked by Weather · gotript`;
  ...
  if (parsed.kind === 'itinerary') {
    const title = `${parsed.days}-Day ${parsed.city.name} Itinerary · gotript`;
  ...
  if (parsed.kind === 'weekend') {
    const title = `A Weekend in ${parsed.city.name} · 2-Day Plan · gotript`;
  ...
  if (parsed.kind === 'comparison') {
    ...
    const title = `${a.name} vs ${b.name}: which to pick · gotript`;
  ...
  if (parsed.kind === 'hotels-in') {
    const title = `Hotels in ${parsed.city.name}, ${parsed.city.countryName} · gotript`;
    const description = `Find hotels, apartments and vacation rentals in ${parsed.city.name}, ${parsed.city.countryName}. Powered by Expedia.`;
    return {
      title,
      description,
      alternates: { canonical },
  ...
  if (parsed.kind === 'flights-to') {
    const title = `Cheap flights to ${parsed.city.name}, ${parsed.city.countryName} · gotript`;
  ...
  if (parsed.kind === 'cars-in') {
    const title = `Car rentals in ${parsed.city.name}, ${parsed.city.countryName} · gotript`;
  ...
  if (parsed.kind === 'hotels-themed') {
    ...
    const title = `${heading} in ${city.name}, ${city.countryName} · gotript`;
    const description = `${heading} in ${city.name}, ${city.countryName} — real Expedia guest reviews. ${HOTEL_THEME_TAGLINE[theme]}`;
    return {
      title,
      description,
      alternates: { canonical },
  ...
  if (parsed.kind === 'flights-themed') {
    const { city } = parsed;
    const title = `Cheap flights to ${city.name}, ${city.countryName} · gotript`;
  ...
  if (parsed.kind === 'cars-themed') {
    ...
    const title = `${label} in ${city.name}, ${city.countryName} · gotript`;
    const description = theme === 'cheap'
      ? `Rental cars in ${city.name}. Every major brand on Expedia Cars, compared side by side.`
  ...
  if (parsed.kind === 'things-themed') {
    ...
    const title = `${label} in ${city.name}, ${city.countryName} · gotript`;
    const description = `${label} in ${city.name}, ${city.countryName} — bookable through Expedia Attractions. Skip-the-line options and verified guides.`;
    return {
      title,
      description,
      alternates: { canonical },
  ...
  // things-to-do
  const title = `Things to do in ${parsed.city.name}, ${parsed.city.countryName} · gotript`;
    ...
    return (
      <>
        ...
        <BestTimeSeoPage
          ...
          bookCta={{
            label: `Find vacation homes in ${parsed.city.name}`,
            href: buildExpediaCategoryUrl('vacation-rentals', {
              destination: `${parsed.city.name}, ${parsed.city.countryName}`,
            }),
            external: true,
            blurb: `Book ${parsed.city.name} vacation rentals — whole homes with kitchens and space — through VRBO and Expedia.`,
          }}
        />
      </>
    );
