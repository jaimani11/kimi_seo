 ...
 *                                     a specific product)
 *   - dpl_turn=<turnId?>             (gotript-private — round-trips
...
export function decorateOutboundUrl(url: string, opts: DecorateOutboundOptions): string {
  ...
  setIfAbsent(parsed.searchParams, 'utm_source', 'gotript');
