 ...
 * params + `_src=gotript` for our own analytics.
 *
...
export function getExpediaMultiConfig(): ExpediaMultiConfig {
  ...
  return {
    ...
    label: labelRaw.length > 0 ? labelRaw : 'gotript',
...
function withAffiliate(url: string, config: ExpediaMultiConfig): string {
  ...
  u.searchParams.set('_src', 'gotript');
  return u.toString();
}

/**
 * Display label + ordering for the 6-tab category strip on the home
 * page. Single source of truth for "which categories does gotript
 * cover and in what order."
 *
 * Compared to gobookt's 5 tabs, gotript adds **Packages** (Expedia's
