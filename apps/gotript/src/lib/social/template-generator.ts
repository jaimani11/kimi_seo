...
const CTA_OPTIONS = [
  'Plan your trip on gotript.com',
  'Build the itinerary at gotript.com',
  'Free AI trip planner → gotript.com',
  'Get the full guide on gotript.com',
  'Bookable on Viator via gotript.com',
];
  ...
  if (guide) {
    ...
    pins.push({
      ...
      description: guide.neighborhoods
        ...
        .join(' ')
        .slice(0, 480),
      visualConcept: `Stylized map of ${city.name} with neighborhoods highlighted in gotript brand colors, plus a small landmark icon per neighborhood.`,
