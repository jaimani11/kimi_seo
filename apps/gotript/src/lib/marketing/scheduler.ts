  ...
  await Promise.all(
    platforms.map(async (platform) => {
      ...
      for (let i = 0; i < slugs.length; i++) {
        ...
        // Rewrite the CTA to embed the branded gotript.com URL +
...
/**
 * Rewrite the CTA on a payload to point at gotript.com with a
 * UTM-tagged link. The pack generator's templates and the LLM both
 * include gotript.com mentions in the CTA text, but we
...
function withBrandedCta(
  ...
  // sometimes embed gotript.com without UTM. Re-attach the
  // tagged URL fresh.
  const stripped = original.replace(/\s*(https?:\/\/)?(www\.)?gotript\.com[^\s]*$/i, '').trim();
