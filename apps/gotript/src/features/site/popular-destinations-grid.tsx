 ...
 * visual weight. gotript's roster is deliberately its OWN (first-time /
 * bucket-list planning cities) so no two brands' grids look alike; photos
 * resolve per-city from @adored/imagery. Each tile deep-links to that city's
 * full planning guide.
 */

const ROSTER: readonly string[] = [
  'paris', 'rome', 'tokyo', 'london', 'new-york', 'barcelona',
  'kyoto', 'istanbul', 'prague', 'sydney', 'amsterdam', 'cairo',
];
      ...
      <div className="mx-auto max-w-6xl px-6 py-14 md:py-20">
        <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div className="max-w-xl">
            ...
            >
              Every destination gets a full guide — weather, neighborhoods and when to go:
            </p>
            <ul
              className="mt-3 flex flex-wrap gap-2"
              style={{ listStyle: 'none', padding: 0, margin: 0 }}
            >
              {['Best time to go', 'Interactive map', 'Neighborhoods & itineraries'].map(
                (feat) => (
                  <li
                    key={feat}
                    style={{
