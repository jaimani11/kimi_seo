...
describe('summarizeKpis', () => {
  ...
  it('counts clicks/bookings/sessions/providers correctly', () => {
    const clicks = [
      ...
      click({ sessionId: 's2', providerId: 'expedia', stayId: 'a', createdAt: '2026-06-02T00:00:00Z' }),
    ];
...
describe('providerCtr', () => {
  it('sorts by click count descending and computes share%', () => {
    const clicks = [
      ...
      click({ sessionId: 's2', providerId: 'expedia', stayId: 'x', createdAt: '2026-06-01T03:00:00Z' }),
    ];
    const rows = providerCtr(clicks);
    expect(rows).toEqual([
      { providerId: 'viator', clicks: 3, sharePct: 75 },
      { providerId: 'expedia', clicks: 1, sharePct: 25 },
    ]);
  });
...
describe('topStays', () => {
  it('groups by (providerId, stayId) and returns top N descending', () => {
    const clicks = [
      ...
      click({ sessionId: 's4', providerId: 'expedia', stayId: 'a', createdAt: '2026-06-01T03:00:00Z' }),
    ];
