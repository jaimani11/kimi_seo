...
- **Expedia Affiliate Partner Network** - already wired via the
  existing `ExpediaProvider` + `bookingLink.url` (uses
  affiliate-deeplinks). The `<ExpediaCta>` resolver falls back to
  destination Expedia search even for Expedia listings, so users
  always have a Expedia option even if Expedia declined.
