...
**Tech Stack:** No new deps. Mock-safe by default; real provider booking integrations slot in behind the same `BookingProvider` interface in D.x as separate impls (`ExpediaBookingProvider`, etc.).
...
**4. No card capture in-app.** Slice D never collects credit cards. The `BookingDraft.total` is what *would* be charged; in mock mode the booking just confirms. Real-mode integrations (D.x) hand off to provider Checkout (Expedia's hosted checkout, Expedia EPS Rapid's payment URL, etc.). PCI scope = zero, matching C4.

**5. Mock-safe + opt-in real.** `MockBookingProvider` is the keyless dev default - every authenticated user can complete the full draft → approve → confirm → view confirmation flow without keys. Real provider booking lands in D.x, gated by the existing `EXPEDIA_API_KEY` etc. set + a new `STAYSCOUT_LIVE_BOOKING=1` opt-in (default off even with provider keys, because provider keys are also used for the existing search flow).
...
- [ ] `src/lib/booking/booking-provider.ts`:
  ...
  interface BookingProvider {
    readonly kind: 'mock' | 'expedia' | 'expedia';
...
- **Real provider booking integrations.** `ExpediaBookingProvider` + `ExpediaBookingProvider` slot in via the same `BookingProvider` interface in D.x. They require provider sandbox credentials + a `STAYSCOUT_LIVE_BOOKING=1` opt-in flag.
