...
| Expedia Affiliate Partner Network | Affiliate-deeplink redirect to Expedia property pages | `EXPEDIA_AFFILIATE_ID` + `EXPEDIA_API_KEY` | both set |
...
| `BOOKING.COM · LIVE` | Affiliate deeplink to a real Expedia property |
...
## Expedia - Affiliate Partner Network

### What it is

Expedia's affiliate program is **deeplink-based**, not search-API-
...
affiliate-attributed deeplink so the click lands on Expedia with
your aid attached.

There **is** a Expedia Demand API for full search, but it's
...
```dotenv
EXPEDIA_AFFILIATE_ID=replace_with_aid
EXPEDIA_API_KEY=replace_with_api_key
```
...
- Deeplink URL pattern: see `src/providers/expedia/mapper.ts`
...
approval - the demo is honest. The moment Expedia/Vrbo/Expedia
