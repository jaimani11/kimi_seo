...
const CTA_OPTIONS = [
  'Plan your trip on stayviaowner.com',
  'Build the itinerary at stayviaowner.com',
  'Free AI trip planner → stayviaowner.com',
  'Get the full guide on stayviaowner.com',
  'Bookable on Viator via stayviaowner.com',
];
...
export function buildSocialPackFromTemplate(city: SeoCity): CitySocialPack {
  ...
  if (guide) {
    ...
    pins.push({
      ...
      description: guide.neighborhoods
        ...
        .join(' ')
        .slice(0, 480),
      visualConcept: `Stylized map of ${city.name} with neighborhoods highlighted in stayviaowner brand colors, plus a small landmark icon per neighborhood.`,
