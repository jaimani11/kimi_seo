 ...
 * route to Expedia via the right vertical (hotels, flights, cars,
 * attractions, cruises).
 *
...
// Expedia vertical landings (one per city).
...
// Hotel-themed sub-shapes — each routes to Expedia hotels with a
...
// Things-to-do themed sub-shapes — these route to Expedia
...
export function parseSeoSlug(slug: string): SeoRouteMatch | null {
  ...
  // Expedia base verticals (one per city).
