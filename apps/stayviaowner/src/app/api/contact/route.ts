...
const CONTACT_FROM_EMAIL =
  process.env.CONTACT_FROM_EMAIL || 'stayviaowner Contact <onboarding@resend.dev>';
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const emailBody = [
    `New contact-form submission on stayviaowner.com`,
    ``,
    `From: ${name} <${email}>`,
    `Subject: ${subject}`,
    ``,
  ...
  const resp = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [CONTACT_TO_EMAIL],
      reply_to: email,
      subject: `[stayviaowner contact] ${subject}`,
      text: emailBody,
    }),
  });
