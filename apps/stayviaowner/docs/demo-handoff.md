...
| Provider router | MockItaly + LLMSynthesized | `EXPEDIA_*` / `EXPEDIA_*` keys | Real provider search via the same router; mocks become fallback. |
...
- [ ] Real `ExpediaBookingProvider` impl (D.x).
