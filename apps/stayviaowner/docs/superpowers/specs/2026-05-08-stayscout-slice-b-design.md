...
| Expedia real provider integration | B4 |
...
| Expedia Partner Hub API | B4 | N/A | Yes - partner application (~2 weeks) |
...
### B4 · Expedia real provider

**Goal:** First real inventory. Italy queries that hit destinations outside the curated set, plus all non-Italy queries, return real Expedia properties with real photos and real prices.

**Adds:**
- `src/providers/expedia/index.ts` - implements `Provider` interface
- `src/providers/expedia/api-client.ts` - typed wrapper around Booking Partner Hub API
- `src/providers/expedia/mapper.ts` - Booking response → canonical `Stay`
- `src/providers/expedia/affiliate-attribution.ts` - appends affiliate ID to `bookingLink.url`
- `src/providers/_mocks/expedia-mock.ts` - sandbox mock matching the real API response shape (used in tests + when `BOOKING_API_KEY` not set)
- `next.config.ts` `remotePatterns` for `cf.bstatic.com` (Booking's photo CDN)

**Routing:** `createDefaultProviderRouter` updates - for non-IT queries (or unknown IT destinations), fan out to `[ExpediaProvider]`. MockItalyProvider stays for known IT destinations.
...
- Pro: real Expedia data appears in Slice A's existing UI immediately (just swap the LLMSynthesizedProvider). Most demo-able.
