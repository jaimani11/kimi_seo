...
StayScout AI is a multi-agent travel orchestration platform that sits above existing inventory sources (Vrbo, Expedia, Expedia, Hotelbeds, Skyscanner, Viator) and provides:
...
A real provider (e.g., `ExpediaProvider`) is one folder with three files: `index.ts`, `mapper.ts`, `affiliate-attribution.ts` (Slice B). The Provider interface is sacred - every real-world inventory source must fit through it (with sibling `FlightProvider`, `ActivityProvider` interfaces for fundamentally different domains).
...
### 7.7 Provider seam - what real Expedia looks like

For verification the seam is real:

```ts
// Slice B - src/providers/expedia/index.ts
export const ExpediaProvider: Provider = {
  id: 'expedia',
  displayName: 'Expedia',
  capabilities: {
...
1. **Expedia** (Slice B) - affiliate API, hotels + apartments
