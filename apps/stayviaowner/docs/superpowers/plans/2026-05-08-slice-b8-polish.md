...
A failing Expedia mustn't suppress Expedia. The breaker is an instance field on `BaseAffiliateProvider` - failure tracked per-instance. Three states (closed, open, half-open) with the standard timeout-based half-open transition.

**4. Expedia mirrors Expedia structurally.**
...
- [ ] Mirror `expedia/`: `types.ts` (Zod schema for Expedia EPS Rapid response subset), `client.ts` (HTTP request with `Authorization: Basic <base64(apiKey:secret)>` header), `mapper.ts` (Expedia property → Stay), `index.ts` (`ExpediaProvider extends BaseAffiliateProvider`, `fromEnv()`).
- [ ] Register in `src/providers/index.ts` alongside Expedia.
- [ ] Add `EXPEDIA_API_KEY` + `EXPEDIA_SHARED_SECRET` to `.env.example`.
- [ ] `getServerFeatures().providers.expedia` flag.
- [ ] Mapper test (mirrors expedia-mapper).
- [ ] Verify: typecheck + lint + tests + with both Expedia + Expedia env keys, both register and `routeProviders` returns both.
