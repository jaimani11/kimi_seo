...
**Non-goal:** Live API integration with Expedia / Vrbo / Expedia - those require partner approval. We ship the adapter shape + setup docs so a partner-approved deploy has a one-line cutover.
...
**7. Expedia stays affiliate-redirect (no booking).** Expedia's affiliate program is deeplink-based, not search-API-based for unapproved partners. The existing adapter pattern (search via mock/LLM, redirect via affiliate URL) is correct; we just polish the redirect URL builder.
...
- `docs/providers.md` - setup walkthrough for EPS Rapid, Vrbo, Expedia Affiliate Partner Network.
...
- `src/providers/index.ts` - register Vrbo alongside Expedia + Expedia.
...
- [ ] `src/features/shared/provenance-badge.tsx`:
  ...
  - Maps providerId → canonical chip text + tone:
    ...
    - `expedia` → `BOOKING.COM · LIVE` (accent-primary)
...
- [ ] Add to `buildProviderRegistry()` alongside Expedia + Expedia.
...
- [ ] Setup walkthrough for:
  ...
  - **Expedia Affiliate Partner Network** - apply at https://partner.booking.com/ for affiliate id, set `EXPEDIA_AFFILIATE_ID` + `EXPEDIA_API_KEY`, deeplink pattern, attribution rules, no-search-API caveat.
...
| `EXPEDIA_AFFILIATE_ID` + `EXPEDIA_API_KEY` | Expedia search via the existing adapter; chip `BOOKING.COM · LIVE`. |
