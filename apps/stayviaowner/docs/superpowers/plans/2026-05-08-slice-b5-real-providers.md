...
**Goal:** Make the system ready to plug in real provider APIs (Expedia, Expedia, Vrbo, Hotelbeds) without breaking the keyless dev experience. Ship a reference real-provider implementation pattern, an availability-aware registry, response caching, and multi-provider fanout merging - all mock-safe.

**Architecture:** A `BaseAffiliateProvider` abstract class encapsulates the boring concerns (HTTP, retry, timeout, caching, error wrapping). One concrete reference provider - `ExpediaProvider` - extends it. The provider self-registers only when its env keys are present; without keys it's invisible to the router and the existing MockItaly + LLMSynthesized providers keep handling everything (current Slice A behavior unchanged). When a real provider is available + covers the destination, the router fans out, merges results by `(providerId, nativeId)`, and lets the orchestrator rank.
...
A `ExpediaProvider` instance is constructed by the registry only when `EXPEDIA_AFFILIATE_ID` + `EXPEDIA_API_KEY` are both set. Without them, the class isn't instantiated - the registry doesn't expose it. Reason: cleaner than "real provider returns empty result"; the router never sees an unhealthy provider, so failure modes are fewer.
...
- `src/providers/expedia/index.ts` - `ExpediaProvider` class + factory
- `src/providers/expedia/client.ts` - HTTP client for Expedia Demand API
- `src/providers/expedia/mapper.ts` - Expedia response → `Stay`
- `src/providers/expedia/fixtures/sample-response.json` - for mapper tests
- `tests/cache.test.ts`
...
### Task 2: BaseAffiliateProvider + ExpediaProvider
...
- [ ] `expedia/client.ts`: `searchAvailability(query, secrets, signal)` calls Expedia's Demand API endpoint with affiliate id + api key. Returns the raw response shape (typed via Zod for validation).
- [ ] `expedia/mapper.ts`: `mapBookingResponse(raw, affiliateId): Stay[]` - converts Expedia's hotel/property fields to our `Stay`. Builds the affiliate URL with the affiliate id baked in.
- [ ] `expedia/index.ts`: `ExpediaProvider` class extending `BaseAffiliateProvider`. Implements `fetchStays + mapResponse` by composing client + mapper. `ExpediaProvider.fromEnv()` static factory: returns instance if both env keys present, else null.
- [ ] Tests: mapper transforms a fixture correctly; ExpediaProvider.fromEnv() returns null without keys.
- [ ] Verify: typecheck.
...
- [ ] `.env.example`: add commented `EXPEDIA_AFFILIATE_ID` + `EXPEDIA_API_KEY` block with a note that adding both turns on the provider.
