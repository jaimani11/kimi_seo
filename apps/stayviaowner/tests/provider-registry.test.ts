...
describe('availability-aware provider registry', () => {
  ...
  it('omits Expedia when env keys are missing', () => {
  ...
  it('includes Expedia when both env keys are present', () => {
    ...
    expect(reg.real[0]?.id).toBe('expedia');
    expect(listAvailableRealProviders()).toEqual(['expedia']);
  });
  ...
  it('registers Expedia + Expedia together when both have keys', () => {
