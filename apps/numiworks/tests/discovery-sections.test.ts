...
describe('buildPropertyAffiliateHref', () => {
  // The active-stay-provider abstraction routes to Viator by default
  // (numiworks is a Viator-affiliate platform; the Viator destination
  // search is what we monetize). Viator doesn't sell hotels, so the
  // CTA lands on the destination's experience search instead. The
  // Booking.com and Expedia builders are preserved internally for the
  // one-env-var flip back.
  it('produces a /r/ redirect URL whose payload points at viator.com', () => {
    ...
    expect(payload!.url).toMatch(/^https:\/\/www\.viator\.com\/searchResults\/all\?/);
