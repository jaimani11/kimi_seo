...
describe('getViatorStayLinkConfig', () => {
  ...
  it('defaults mcid to numiworks-stay', () => {
    expect(getViatorStayLinkConfig().mcid).toBe('numiworks-stay');
  });
...
describe('buildViatorStaySearchUrl', () => {
  ...
  it('builds the canonical /searchResults/all URL with the destination', () => {
    ...
    expect(url).toBe(
      'https://www.viator.com/searchResults/all?text=Agra&pid=P00012345&mcid=numiworks-stay&medium=link',
    );
  });
  ...
  it('always includes medium=link and the configured mcid', () => {
    const url = buildViatorStaySearchUrl(
      { destination: 'Rome' },
      { ...baseConfig, mcid: 'numiworks-card' },
    );
    expect(url).toContain('mcid=numiworks-card');
