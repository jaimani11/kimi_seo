...
describe('TOKYO_SAMPLE_PACK', () => {
  ...
  it('every CTA mentions numiworks (per the brief)', () => {
    for (const p of TOKYO_SAMPLE_PACK.pinterest) {
      expect(p.cta.toLowerCase()).toContain('numiworks');
    }
    for (const s of [
      ...
      expect(s.cta.toLowerCase()).toContain('numiworks');
    }
  });
...
describe('Template generator fallback', () => {
  ...
  it('every CTA mentions numiworks', () => {
    const city = findCityBySlug('paris')!;
    const pack = buildSocialPackFromTemplate(city);
    for (const p of pack.pinterest) expect(p.cta.toLowerCase()).toContain('numiworks');
