...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  // Every query-string variant is noindex — search-result pages are
  // low-quality-signal surfaces (near-infinite variants, thin per-query
  // content) and get penalized by Google if crawled at scale. The
  // strong themed + destination pages carry the SEO weight.
  const robots = { index: false, follow: false } as const;
  if (!q) return { title: 'Search · numiworks', robots };
  return {
    title: `${q} · search · numiworks`,
