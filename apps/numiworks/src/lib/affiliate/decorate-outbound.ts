 ...
 *                                     a specific product)
 *   - dpl_turn=<turnId?>             (numiworks-private — round-trips
...
export function decorateOutboundUrl(url: string, opts: DecorateOutboundOptions): string {
  ...
  setIfAbsent(parsed.searchParams, 'utm_source', 'numiworks');
