  ...
  await Promise.all(
    platforms.map(async (platform) => {
      ...
      for (let i = 0; i < slugs.length; i++) {
        ...
        // Rewrite the CTA to embed the branded numiworks.com URL +
...
/**
 * Rewrite the CTA on a payload to point at numiworks.com with a
 * UTM-tagged link. The pack generator's templates and the LLM both
 * include numiworks.com mentions in the CTA text, but we
...
function withBrandedCta(
  ...
  args: { citySlug: string; platform: 'pinterest' | 'instagram' | 'tiktok' },
): MarketingPostPayload {
  const brandedUrl = brandedNumiworksUrl(args);
  const original = (payload as { cta?: string }).cta ?? '';
  // Drop any previously-attached URL on the CTA — the templates
  // sometimes embed numiworks.com without UTM. Re-attach the
  // tagged URL fresh.
  const stripped = original.replace(/\s*(https?:\/\/)?(www\.)?numiworks\.com[^\s]*$/i, '').trim();
